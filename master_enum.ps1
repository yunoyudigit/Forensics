...
# Create filename for HTMLReport
   ...
   [string]$UserHost = $ENV:COMPUTERNAME + $env:USERPROFILE
   [string]$FileName = $StartTime + '_' + $ENV:COMPUTERNAME + '.html'
   $HTMLReportFile = (Join-Path $env:USERPROFILE $FileName)
...
TD {border-width: 1px;padding: 3px;border-style: solid;border-color: black;font-family:courier; width: auto;}
...
$Results | ConvertTo-HTML -Fragment -PreContent "<H2>Interesting Registry Keys</H2>`n<table><tr><td><PRE>$Results</PRE></td></tr></table>" -as list | Out-File -Append $HTMLReportFile
...
"`n[+] Browser History`n"
$Results = Get-BrowserInformation -Browser All -Datatype History -UserName * | Where-Object{$_.Data -NotMatch "google" -And $_.Data -NotMatch "microsoft" -And $_.Data -NotMatch "chrome" -And $_.Data -NotMatch "youtube" }
...
$Results | ConvertTo-HTML -Fragment -PreContent "<H2>Clipboard Contents - $($Env:UserName)</H2><table><tr><td><PRE>$Results</PRE></td></tr></table>"| Out-File -Append $HTMLReportFile
...
Get-ChildItem $Results.AppLockerProcessStart | Select-Object Name,FullName,Parent,Exists,Root,Extension,CreationTime,CreationTimeUtc,LastAccessTime,LastAccessTimeUtc,LastWriteTime,LastWriteTimeUtc,Attributes | ConvertTo-Html -Fragment -PreContent "<H2> AppLocker Process Start </H2>" | Out-File -Append $HTMLReportFile
...
#>
    ...
    $ErrorActionPreference = "SilentlyContinue"            
        ...
        Get-ItemProperty "HKLM:\SYSTEM\CurrentControlSet\services\snmp\parameters\validcommunities" | Format-Table -AutoSize | Out-String
    
        # HKCU SNMP Keys 
        "`n[+] SNMP community strings for current user:`n"
        Get-ItemProperty "HKCU:\SYSTEM\CurrentControlSet\services\snmp\parameters\validcommunities"| Format-Table -AutoSize | Out-String
        ...
        Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Run\*" | Format-Table -AutoSize | Out-String
        Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\RunOnce\*" | Format-Table -AutoSize | Out-String
        Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\Run\*" | Format-Table -AutoSize | Out-String
        Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\RunOnce\*" | Format-Table -AutoSize | Out-String

        # Startup Folder Keys
        "`n[+] Startup folder keys present:`n"
        Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\User Shell Folders\*" | Format-Table -AutoSize | Out-String
        Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders\*" | Format-Table -AutoSize | Out-String
        Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\Explorer\User Shell Folders\*" | Format-Table -AutoSize | Out-String
        Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders\*" | Format-Table -AutoSize | Out-String
        
        # Boot Service Startup Keys
        "`n[+] Boot Service Startup keys present:`n"
        Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\RunServicesOnce\*" | Format-Table -AutoSize | Out-String
        Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\RunServicesOnce\*" | Format-Table -AutoSize | Out-String
        Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\RunServices\*" | Format-Table -AutoSize | Out-String
        Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\RunServices\*" | Format-Table -AutoSize | Out-String
        
        # Created when policy settings specify startup programs
        "`n[+] Policy created startup keys present:`n"
        Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer\Run\*" | Format-Table -AutoSize | Out-String
        Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer\Run\*" | Format-Table -AutoSize | Out-String
