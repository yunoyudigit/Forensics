...
$Results = Get-AVInfo
$Results | ConvertTo-HTML -Fragment -PreContent "<H2>Installed AV Product</H2>" | Out-File -Append $HTMLReportFile
...
If ($Results.displayName -like "*mcafee*") {
    ...
    $Results | ConvertTo-HTML -Fragment -PreContent "<H2>Recent McAfee AV Logs</H2>" | Out-File -Append $HTMLReportFile
}     
...
$Results | ConvertTo-HTML -Fragment -PreContent "<H2>Interesting Registry Keys</H2>`n<table><tr><td><PRE>$Results</PRE></td></tr></table>" | Out-File -Append $HTMLReportFile
...
$Results | ConvertTo-HTML -Fragment -PreContent "<H2>Interesting Files</H2>`n<table><tr><td><PRE>$Results</PRE></td></tr></table>" | Out-File -Append $HTMLReportFile
        
## User Enumeration

# Group Membership for each User

$usernames = Get-WmiObject -Class Win32_UserAccount -Filter "Domain='$($env:ComputerName)'" | Select-Object Name
foreach ($user in $usernames){

    "`n[+] Group Membership - $($user.name)`n"
...
"`n[+] Browser History`n"
$Results = Get-BrowserInformation -Browser All -Datatype History -UserName * | Where-Object{$_.Data -NotMatch "google" -And $_.Data -NotMatch "microsoft" -And `
 $_.Data -NotMatch "chrome" -And $_.Data -NotMatch "youtube" }
...
$usernames = Get-WmiObject -Class Win32_UserAccount -Filter "Domain='$($env:ComputerName)'" | Select-Object Name
foreach ($user in $usernames){
    "`n[+] Active Internet Explorer URLs - $($user.name)`n"
    $Results = Get-ActiveIEURLS
    $Results | ConvertTo-HTML -Fragment -PreContent "<H2>Active Internet Explorer URLs - $($user.name)</H2>" | Out-File -Append $HTMLReportFile
}
#>
...
$usernames = Get-WmiObject -Class Win32_UserAccount -Filter "Domain='$($env:ComputerName)'" | Select-Object Name
foreach ($user in $usernames){
    "`n`n[+] Recycle Bin Contents - $($user.name)`n"
    $Results = Get-RecycleBin
    $Results | ConvertTo-HTML -Fragment -PreContent "<H2>Recycle Bin Contents - $($user.name)</H2>" | Out-File -Append $HTMLReportFile
}
...
$usernames = Get-WmiObject -Class Win32_UserAccount -Filter "Domain='$($env:ComputerName)'" | Select-Object Name
foreach ($user in $usernames){
    "`n[+] Clipboard Contents - $($user.Name):`n"
    $Results = ''
    $Results = ([Windows.Clipboard]::GetText()) -join "`r`n" | Out-String
    $Results | ConvertTo-HTML -Fragment -PreContent "<H2>Clipboard Contents - $($user.Name)`n<table><tr><td><PRE>$Results</PRE></td></tr></table>"| Out-File -Append $HTMLReportFile
}
...
#>
    ...
    $ErrorActionPreference = "SilentlyContinue"            
        ...
        Get-ItemProperty "HKLM:\SYSTEM\CurrentControlSet\services\snmp\parameters\validcommunities" | Out-String
    
        # HKCU SNMP Keys 
        "`n[+] SNMP community strings for current user:`n"
        Get-ItemProperty "HKCU:\SYSTEM\CurrentControlSet\services\snmp\parameters\validcommunities"| Out-String
    
        # Putty Saved Session Keys
        "`n[+] Putty saved sessions:`n"
        Get-ItemProperty "HKCU:\Software\SimonTatham\PuTTY\Sessions\*" | Format-Table -AutoSize | Out-String
        ...
        Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\User Shell Folders\*" | Out-String
        Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders\*" | Out-String
        Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\Explorer\User Shell Folders\*" | Out-String
        Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders\*" | Out-String
        
        # Boot Service Startup Keys
        "`n[+] Boot Service Startup keys present:`n"
        Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\RunServicesOnce\*" | Out-String
        Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\RunServicesOnce\*" | Out-String
        Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\RunServices\*" | Out-String
        Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\RunServices\*" | Out-String
        
        # Created when policy settings specify startup programs
        "`n[+] Policy created startup keys present:`n"
        Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer\Run\*" | Out-String
        Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer\Run\*" |  Out-String
...
#>
    Write-Verbose "Enumerating interesting files..."
        ...
        $IndexedFiles | Out-String
    
        # Get Top Level file listing of all drives
        "`n[+] All 'FileSystem' Drives - Top Level Listing:`n"
        Get-PSdrive -psprovider filesystem | ForEach-Object {gci $_.Root} | Select-Object Fullname,LastWriteTimeUTC,LastAccessTimeUTC,Length | Out-String
    
        # Get Program Files
        "`n[+] System Drive - Program Files:`n"
        gci $ENV:ProgramFiles | Select-Object FullName,LastWriteTimeUTC,LastAccessTimeUTC,Length | Out-String
    
        # Get Program Files (x86)
        "`n[+] System Drive - Program Files (x86):`n"
        gci "$ENV:ProgramFiles (x86)" | Select-Object Fullname,LastWriteTimeUTC,LastAccessTimeUTC,Length | Out-String
    
        # Get %USERPROFILE%\Desktop top level file listing
        "`n[+] Current User Desktop:`n"
        gci $ENV:USERPROFILE\Desktop | Select-Object Fullname,LastWriteTimeUTC,LastAccessTimeUTC,Length | Out-String
    
        # Get %USERPROFILE%\Documents top level file listing
        "`n[+] Current User Documents:`n"
        gci $ENV:USERPROFILE\Documents | Select-Object Fullname,LastWriteTimeUTC,LastAccessTimeUTC,Length | Out-String
        ...
        gci $ENV:USERPROFILE -recurse -include *pass*,*diagram*,*.pdf,*.vsd,*.doc,*docx,*.xls,*.xlsx,*.kdbx,*.key,KeePass.config | Select-Object Fullname,LastWriteTimeUTC,LastAccessTimeUTC,Length | Out-String
    
        # Get Host File
        "`n[+] Contents of Hostfile:`n`n"
        (Get-Content -path "$($ENV:WINDIR)\System32\drivers\etc\hosts") -join "`r`n" | Out-String
    }
...
function Get-SymantecLog
{
<#
.SYNOPSIS

Retrieve the current day's SEP log from the user's machine.

Function: Get-SymantecLog
Author: Jake Gilane, Twitter: @gm4n
Required Dependencies: None
Optional Dependencies: None
...
Get-SymantecLog
Gets the current day's SEP log from the system.
...
#>
    ...
    $LogPath = 'C$\ProgramData\Symantec\Symantec Endpoint Protection\CurrentVersion\Data\Logs\AV'
    $LogEntries = Get-Item -Path $LogPath
