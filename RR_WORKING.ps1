...
RAPID RESPONSE FORENSIC ENUMERATION SCRIPT:

Modified from: Invoke-HostEnum
Credit to @andrewchiles
...
.SYNOPSIS
    ...
    Performs local host enumeration for investigation

    Author: Jake Gilane (@gm4n) leveraging functions by @andrewchiles, @mattifestation, @harmj0y, Joe Bialek, rvrsh3ll, Beau Bullock, and Tim Medin
...
.DESCRIPTION
    ...
    Script is Powershell 2.0 compatible to ensure it functions on the widest variety of Windows targets
...
$HTMLReportFile = 'C:\' + $ENV:COMPUTERNAME + '.html'
...
TH {border-width: 1px;padding: 3px;border-style: solid;border-color: black;background-color: #6495ED;text-align: left;}
TD {border-width: 1px;padding: 3px;border-style: solid;border-color: black;font-family:courier; width: auto;text-align: left;}
TR:Nth-Child(Even) {Background-Color: #dddddd;padding: 3px;}
...
Try {
    ...
    -Body "<H1>System Enumeration Report for $($ENV:COMPUTERNAME)</H1>`n<div class='aLine'></div>" `
...
# Execute local system enumeration functions and format for report
...
$os_info = gwmi Win32_OperatingSystem -ComputerName $env:COMPUTERNAME
...
$SysInfoHash = @{            
    ...
    IPADDRESSES             = (@([System.Net.Dns]::GetHostAddresses($env:COMPUTERNAME)) | %{$_.IPAddressToString}) -join ", "        
...
$Results = New-Object -TypeName PSobject -Property $SysInfoHash | Select-Object Hostname, OS, Architecture, "Date(UTC)", "Date(Local)", InstallDate, UpTime, IPAddresses, Domain, Username, LogonServer, PSVersion, PSScriptBlockLogging, PSTranscription, PSTranscriptionDir
$Results | ConvertTo-HTML -Fragment -PreContent "<H2>Host Summary</H2>" | Out-File -Append $HTMLReportFile
...
gwmi win32_process -ComputerName $ENV:COMPUTERNAME |% {$owners[$_.handle] = $_.getowner().user}
gwmi win32_process -ComputerName $ENV:COMPUTERNAME |% {$commandline[$_.handle] = $_.commandline}

$procs = Get-Process -ComputerName $ENV:COMPUTERNAME | Sort-Object -property ID
...
$procs | ConvertTo-HTML -Fragment -Property ID, Name, Owner, MainWindowTitle, Path, CommandLine -PreContent "<H2>Process Information</H2>" | Out-File -Append $HTMLReportFile
...
$Results | ConvertTo-HTML -Fragment -PreContent "<H2>BIOS Information</H2>" | Out-File -Append $HTMLReportFile
...
$Results | ConvertTo-HTML -Fragment -PreContent "<H2>Physical Computer Information</H2>" | Out-File -Append $HTMLReportFile
...
$Results = Get-WmiObject -class Win32_NetworkAdapterConfiguration | 
    Select-Object Description,@{Label="IPAddress";Expression={($_.IPAddress) -join ", "}},@{Label="IPSubnet";Expression={($_.IPSubnet) -join ", "}},@{Label="DefaultGateway";Expression={($_.DefaultIPGateway) -join ", "}},MACaddress,DHCPServer,DNSHostname | 
    Sort-Object IPAddress -descending
...
$Connections | Select-Object LocalAddress, LocalPort, RemoteAddress, RemotePort, State, IPVersion
$Connections | ConvertTo-HTML -Fragment -PreContent "<H2>Active TCP Connections</H2>" | Out-File -Append $HTMLReportFile
...
$Results = ForEach($Connection in $UdpListeners) {            
   if($Connection.address.AddressFamily -eq "InterNetwork" ) { $IPType = "IPv4" } else { $IPType = "IPv6" }                 
   New-Object -TypeName PSobject -Property @{          
        "Protocol"      = "UDP"
        "LocalAddress"  = $Connection.Address            
...
$Results = Select-Object | Where-Object {$_.ListeningPort -LT 50000} | Select-Object Protocol, LocalAddress, ListeningPort, IPVersion
...
$Results = @()    
    ...
    Get-Itemproperty "HKCU:\software\microsoft\windows\currentversion\explorer\runmru" | ConvertTo-HTML -Fragment -PreContent "<H2>Recent RUN Commands</H2>`n<table><tr><td><PRE></PRE></td></tr></table>" | Out-File -Append $HTMLReportFile

    # HKLM SNMP Keys
    "`n[+] SNMP community strings:`n"
    Get-ItemProperty "HKLM:\SYSTEM\CurrentControlSet\services\snmp\parameters\validcommunities" | ConvertTo-HTML -Fragment -PreContent "<H2>SNMP Community Strings</H2>`n<table><tr><td><PRE></PRE></td></tr></table>" | Out-File -Append $HTMLReportFile
    
    # HKCU SNMP Keys 
    "`n[+] SNMP community strings for current user:`n"
    Get-ItemProperty "HKCU:\SYSTEM\CurrentControlSet\services\snmp\parameters\validcommunities"| ConvertTo-HTML -Fragment -PreContent "<H2>Current User SNMP Strings</H2>`n<table><tr><td><PRE></PRE></td></tr></table>" | Out-File -Append $HTMLReportFile
    
    # Putty Saved Session Keys
    "`n[+] Putty saved sessions:`n"
    Get-ItemProperty "HKCU:\Software\SimonTatham\PuTTY\Sessions\*" | ConvertTo-HTML -Fragment -PreContent "<H2>PuTTy Saved Session Keys</H2>`n<table><tr><td><PRE></PRE></td></tr></table>" | Out-File -Append $HTMLReportFile

    #Default Run Keys
    "`n[+] Any RUN keys present:`n"
    $Results += Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Run\*" | Out-String
    $Results += Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\RunOnce\*" | Out-String
    $Results += Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\Run\*" | Out-String
    $Results += Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\RunOnce\*" | Out-String
    $Results | ConvertTo-HTML -Fragment -PreContent "<H2>Default Run Keys</H2>`n<table><tr><td><PRE>$Results</PRE></td></tr></table>" | Out-File -Append $HTMLReportFile 

    # Startup Folder Keys
    "`n[+] Startup folder keys present:`n"
    $Results = @()
    $Results += Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\User Shell Folders\*" | Out-String
    $Results += Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders\*" | Out-String
    $Results += Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\Explorer\User Shell Folders\*" | Out-String
    $Results += Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders\*" | Out-String
    $Results | ConvertTo-HTML -Fragment -PreContent "<H2>Startup Folder Keys</H2>`n<table><tr><td><PRE>$Results</PRE></td></tr></table>" | Out-File -Append $HTMLReportFile
        
    # Boot Service Startup Keys
    "`n[+] Boot Service Startup keys present:`n"
    $Results = @()
    $Results += Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\RunServicesOnce\*" | Out-String
    $Results += Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\RunServicesOnce\*" | Out-String
    $Results += Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\RunServices\*" | Out-String
    $Results += Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\RunServices\*" | Out-String
    $Results | ConvertTo-HTML -Fragment -PreContent "<H2>Boot Service Startup Keys</H2>`n<table><tr><td><PRE>$Results</PRE></td></tr></table>" | Out-File -Append $HTMLReportFile
        
    # Created when policy settings specify startup programs
    "`n[+] Policy created startup keys present:`n"
    $Results = @()
    $Results += Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer\Run\*" | Out-String
    $Results += Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer\Run\*" |  Out-String
    $Results | ConvertTo-HTML -Fragment -PreContent "<H2>Policy Startup Keys</H2>`n<table><tr><td><PRE>$Results</PRE></td></tr></table>" | Out-File -Append $HTMLReportFile
...
$Results = ""
$users = Get-WmiObject -Class Win32_UserAccount -Filter "Domain='$($env:ComputerName)'" | Select-Object Name
    
# Get Top Level file listing of all drives
$Results = Get-PSdrive -psprovider filesystem | ForEach-Object {gci $_.Root} | Select-Object Fullname,LastWriteTimeUTC,LastAccessTimeUTC,Length
$Results | ConvertTo-HTML -Fragment -PreContent "<H2>Top Level Files</H2>`n<table><tr><td><PRE>$Results</PRE></td></tr></table>" | Out-File -Append $HTMLReportFile
    
# Get Program Files
$Results = gci $ENV:ProgramFiles | Select-Object FullName,LastWriteTimeUTC,LastAccessTimeUTC,Length
$Results | ConvertTo-HTML -Fragment -PreContent "<H2> Program Files</H2>`n<table><tr><td><PRE>$Results</PRE></td></tr></table>" | Out-File -Append $HTMLReportFile
    
# Get Program Files (x86)
$Results = gci "$ENV:ProgramFiles (x86)" | Select-Object Fullname,LastWriteTimeUTC,LastAccessTimeUTC,Length
$Results | ConvertTo-HTML -Fragment -PreContent "<H2>Program Files (x86)</H2>`n<table><tr><td><PRE>$Results</PRE></td></tr></table>" | Out-File -Append $HTMLReportFile

foreach ($user in $users){
    # Get %USERPROFILE%\Desktop top level file listing
    $Results = gci "C:\users\$($user.Name)\Desktop" | Select-Object Fullname,LastWriteTimeUTC,LastAccessTimeUTC,Length
    $Results | ConvertTo-HTML -Fragment -PreContent "<H2>User Desktop Files for $($user.Name)</H2>" | Out-File -Append $HTMLReportFile
    
    # Get %USERPROFILE%\Documents top level file listing
    $Results = gci "C:\users\$($user.Name)\Documents" | Select-Object Fullname,LastWriteTimeUTC,LastAccessTimeUTC,Length
    $Results | ConvertTo-HTML -Fragment -PreContent "<H2>User Documents for $($user.Name)</H2>" | Out-File -Append $HTMLReportFile
    ########################   FIX  ###############################
    # Get Files in the %USERPROFILE% directory with certain extensions or phrases specified in $SearchStrings var
    $Results = gci "C:\users\$($user.Name)" -recurse -include $SearchStrings | Select-Object Fullname,LastWriteTimeUTC,LastAccessTimeUTC,Length
    $Results | ConvertTo-HTML -Fragment -PreContent "<H2>User Profile Targeted Files for $($user.Name)</H2>" | Out-File -Append $HTMLReportFile
}
    
# Get Host File
$Results = (Get-Content -path "$($ENV:WINDIR)\System32\drivers\etc\hosts") -join "`r`n"
ConvertTo-HTML -Fragment -PreContent "<H2>Hostfile Contents</H2>`n<table><tr><td><PRE>$Results</PRE></td></tr></table>" | Out-File -Append $HTMLReportFile

# Group Membership for each Local User
$users = Get-WmiObject -Class Win32_UserAccount -Filter "Domain='$($env:ComputerName)'" | Select-Object Name
foreach ($user in $users){
    $Results = net user $user.Name | Select-String -SimpleMatch 'Local Group Memberships'
    $Results | ConvertTo-HTML -Fragment -PreContent "<H2>Group Membership - $($user.Name)</H2>"| Out-File -Append $HTMLReportFile
}
...
$Output | ConvertTo-HTML -Fragment -PreContent "<H2>Recycle Bin Contents</H2>" | Out-File -Append $HTMLReportFile

       
# Clipboard Contents (WIP, unsure on functionality yet
Add-Type -Assembly PresentationCore
$Results = ''
$Results = ([Windows.Clipboard]::GetText()) -join "`r`n" | Out-String
ConvertTo-HTML -Fragment -PreContent "<H2>Clipboard Contents</H2>`n<table><tr><td><PRE>$Results</PRE></td></tr></table>" | Out-File -Append $HTMLReportFile
####    FIX THIS!!! FORMATTING SUCKS, ALL ONE LINE    ####
# Recent CLI History

$Results = (Get-History -Count 20).CommandLine
$Results.ToString() | ConvertTo-HTML -Fragment -PreContent "<H2>Recent CLI History</H2>`n<table><tr><td><PRE>$Results</PRE></td></tr></table>"| Out-File -Append $HTMLReportFile
        
# Interesting Windows Log Parse
$Results = (Get-ComputerDetails) | Select-Object LogonEvent4624,LogonEvent4648,AppLockerProcessStart,PowerShellScriptStart,RdpClientData | Sort-Object name
$Results.LogonEvent4624 | Select-Object LogSource, Count, SourceAccountName, LogType, TargetServer, TargetAccountName, SourceDomainName, TargetDomainName | ConvertTo-Html -Fragment -PreContent "<H2> Windows 4624 Logon Events </H2>" | Out-File -Append $HTMLReportFile
$Results.LogonEvent4648 | Select-Object LogSource, Count, SourceAccountName, LogType, TargetServer, TargetAccountName, SourceDomainName, TargetDomainName | ConvertTo-Html -Fragment -PreContent "<H2> Windows 4648 Logon Events </H2>" | Out-File -Append $HTMLReportFile
$Results.PowerShellScriptStart | Select-Object UserName,Times,ScriptName,Count | ConvertTo-Html -Fragment -PreContent "<H2> PowerShell Scripts Started </H2>" | Out-File -Append $HTMLReportFile
Get-ChildItem $Results.AppLockerProcessStart | Select-Object Name,FullName,Parent,Root,Extension,CreationTimeUtc,LastAccessTimeUtc,LastWriteTimeUtc,Attributes | ConvertTo-Html -Fragment -PreContent "<H2> AppLocker Process Start </H2>" | Out-File -Append $HTMLReportFile
...
#>
    ...
    function Get-ChromeHistory {
        ...
        $AllUsers = Get-WmiObject -Class Win32_UserAccount -Filter "Domain='$($env:ComputerName)'" | Select-Object Name

        foreach ($UserName in $AllUsers){
            $Path = "$Env:systemdrive\Users\$($UserName.name)\AppData\Local\Google\Chrome\User Data\Default\History"
            if (-not (Test-Path -Path $Path)) {
                Write-Verbose "[-] Could not find Chrome History for username: $($UserName.name)"
            }
            $Regex = '(http|ftp|https|file)://([\w_-]+(?:(?:\.[\w_-]+)+))([\w.,@?^=%&:/~+#-]*[\w@?^=%&/~+#-])?'
            $Value = Get-Content -Path "$Env:systemdrive\Users\$($UserName.name)\AppData\Local\Google\Chrome\User Data\Default\History"|Select-String -AllMatches $regex |% {$_.Matches}
    ...
    function Get-ChromeBookmarks {
        ...
        foreach ($UserName in $AllUsers){
            $Path = "$Env:systemdrive\Users\$($UserName.name)\AppData\Local\Google\Chrome\User Data\Default\Bookmarks"
            if (-not (Test-Path -Path $Path)) {
                Write-Verbose "[-] Could not find Chrome Bookmarks for username: $($UserName.name)"
            }   
            else
            {
